...
public class Quadrangle extends Shape {
    ...
    public Coordinate3D getIntersectionPoint(Coordinate3D start, Coordinate3D end) {
        ...
        if (Math.abs(s - s1 - s2 - s3) < EXP) {
            ...
            deltaX = Math.max(Math.max(point4.x, point2.x), point3.x) - Math.min(Math.min(point4.x, point2.x), point3.x);
            deltaY = Math.max(Math.max(point4.y, point2.y), point3.y) - Math.min(Math.min(point4.y, point2.y), point3.y);
            deltaZ = Math.max(Math.max(point4.z, point2.z), point3.z) - Math.min(Math.min(point4.z, point2.z), point3.z);
            if (deltaZ <= deltaX && deltaZ <= deltaY) {
                s = getArea(point4.x, point4.y, point2.x, point2.y, point3.x, point3.y);
                s1 = getArea(point4.x, point4.y, point2.x, point2.y, intersection.x, intersection.y);
                s2 = getArea(point4.x, point4.y, intersection.x, intersection.y, point3.x, point3.y);
                s3 = getArea(intersection.x, intersection.y, point2.x, point2.y, point3.x, point3.y);
            } else if (deltaY <= deltaX && deltaY <= deltaZ) {
                s = getArea(point4.x, point4.z, point2.x, point2.z, point3.x, point3.z);
                s1 = getArea(point4.x, point4.z, point2.x, point2.z, intersection.x, intersection.z);
                s2 = getArea(point4.x, point4.z, intersection.x, intersection.z, point3.x, point3.z);
                s3 = getArea(intersection.x, intersection.z, point2.x, point2.z, point3.x, point3.z);
            } else {
                s = getArea(point4.z, point4.y, point2.z, point2.y, point3.z, point3.y);
                s1 = getArea(point4.z, point4.y, point2.z, point2.y, intersection.z, intersection.y);
